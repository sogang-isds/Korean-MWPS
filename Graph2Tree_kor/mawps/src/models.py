...
def make_pretrained_embedding(embedding_size,input_lang,vectors ):
    ...
    with open(vectors, "r") as f:
        for l in tqdm(f):
            ...
            glove2vec[word] = np.array(line[1:]).astype(np.float)
    word2vec = {}
    word_arr = input_lang.word2index.keys()

    for w in tqdm(word_arr):
        if w in glove2vec.keys():
            word2vec[w] = glove2vec[w]
    embedding_dim = embedding_size
    weight_matrix = torch.zeros((input_lang.n_words, embedding_dim), dtype=torch.float)
...
def create_emb_layer(weights_matrix, non_trainable=True):
    ...
    # emb_layer.weight = weights_matrix
